 ...
 * @version $Id: ReportJobAclDao.java 8408 2007-05-29 23:29:12Z melih $
 */
public class ReportJobAclDao implements BasicAclDao {
	...
	public BasicAclEntry[] getAcls(AclObjectIdentity objectIdentity) {
		...
		NamedEntityObjectIdentity namedId = (NamedEntityObjectIdentity) objectIdentity;
		long jobId = Long.parseLong(namedId.getId());
		String jobOwner = getReportJobsInternalService().getJobOwner(jobId);
		SimpleAclEntry userReadEntry = new SimpleAclEntry(jobOwner, objectIdentity, null, SimpleAclEntry.READ_WRITE_DELETE);
		
		SimpleAclEntry admReadEntry = new SimpleAclEntry(getAdministratorRole(), objectIdentity, null, SimpleAclEntry.ADMINISTRATION);		
