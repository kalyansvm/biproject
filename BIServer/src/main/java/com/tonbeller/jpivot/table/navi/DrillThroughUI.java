    ...
    public Element build(Cell cell, boolean even) {
      ...
      if (canDrillThrough(cell) && (!cell.isNull())) {
        ...
        elem.setAttribute("title", 
        		resources.getString("jsp.jpivot.showSourceData"));
  ...
  class DrillThroughHandler implements RequestListener {
    ...
    public void request(RequestContext context) throws Exception {
      if (canDrillThrough(cell)) {
        ...
        // i18n: get session locale for ui display
        Locale locale = (Locale) session.getAttribute("locale");
        if (cell instanceof MondrianCell) {
        	MondrianCell monCell = (MondrianCell) cell;
        	((MondrianModel) monCell.getModel()).setLocale(locale);
        }
        else if (cell instanceof XMLA_Cell) {
        	// TODO drill through not implemented in XMLA
        }
