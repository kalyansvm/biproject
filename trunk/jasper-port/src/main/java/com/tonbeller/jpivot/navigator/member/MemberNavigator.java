...
public class MemberNavigator extends TreeComponent implements ModelChangeListener, Available {
  ...
  public static final String MEMBER_NAVIGATOR_LAZY_FETCH_CHILDREN = "jsp.jpivot.MemberNavigator.lazyFetchChildren";
  public static final String MEMBER_NAVIGATOR_EXPAND_SELECTED = "jsp.jpivot.MemberNavigator.expandSelected";
  public static final String MEMBER_NAVIGATOR_INITIAL_GROUPING = "jsp.jpivot.MemberNavigator.initialGrouping";
  public static final String MEMBER_NAVIGATOR_GROUPING_MEMBER_COUNT = "jsp.jpivot.MemberNavigator.groupingMemberCount";
  ...
  private void renderButtons(Element parent, Document factory) {
    ...
    Button.addButton(buttons, okButtonId, resources.getString("jsp.jpivot.MemberNavigator.ok.title"));
    Button.addButton(buttons, cancelButtonId, resources.getString("jsp.jpivot.MemberNavigator.cancel.title"));
  }

  private void appendGroupingButton(Element buttons) {
    if (isGrouping()) {
      String label = resources.getString("jsp.jpivot.MemberNavigator.disableGrouping.title");
      Button.addButton(buttons, disableGroupingButtonId, label);
    } else {
      String label = resources.getString("jsp.jpivot.MemberNavigator.enableGrouping.title");
  ...
  private void appendSelectButton(Element buttons) {
    ...
    if (getSelectionModel().getMode() == SelectionModel.MULTIPLE_SELECTION) {
      if (getSelectionModel().isEmpty()) {
        String label = resources.getString("jsp.jpivot.MemberNavigator.selectVisible.title");
        Button.addButton(buttons, selectVisibleButtonId, label);
      } else {
        String label = resources.getString("jsp.jpivot.MemberNavigator.selectNone.title");
  ...
  private TreeModelAdapter.OverflowListener overflowListener = new TreeModelAdapter.OverflowListener() {
    public void overflowOccured() {
      setError(resources.getString("jsp.jpivot.MemberNavigator.overflowOccurred"));
    }
  };
