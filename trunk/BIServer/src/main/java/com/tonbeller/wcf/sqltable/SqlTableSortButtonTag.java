  ...
  private String getSortButtonImageSource(SqlTable table) throws JspException {
    ...
    if (currentColumn.equals(column)) {
      if (table.isDescending())
        return res.getString("jsp.wcf.sqltable.sortimg.cur.desc", contextPath);
      return res.getString("jsp.wcf.sqltable.sortimg.cur.asc", contextPath);
    }
    if (table.isDescending())
      return res.getString("jsp.wcf.sqltable.sortimg.other.desc", contextPath);
    return res.getString("jsp.wcf.sqltable.sortimg.other.asc", contextPath);
  }
