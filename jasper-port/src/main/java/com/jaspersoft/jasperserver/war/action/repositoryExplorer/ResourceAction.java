...
public class ResourceAction extends FormAction {
	...
	public ResourceListModel returnModel(List resources, RequestContext context, int pageNumber) {
		...
		for (int i=(pagination*pageNumber); i<upperLimit ; i++) {
			...
			List jobs = schedulingService.getScheduledJobs(exContext(context), res.getURIString());
			if (jobs.size() > 0) {
			...
			if (resourceType.endsWith("AdhocReportUnit") || resourceType.endsWith("ReportUnit")) {
				...
				row.setResourceUrl("olap/viewOlap.html?name=" + res.getURIString() + "&new=true&parentFlow=repositoryFlow" + "&ParentFolderUri=" + res.getParentURI().substring(5));
	...
	public Event updateResourceProperties(RequestContext context) {
		...
		try {
			...
			String idProperty = (String) context.getRequestParameters().get("id");
			String folderUri = (String) context.getRequestParameters().get("folderUri");
	...
	public Event getResourceProperties(RequestContext context) {
		...
		try {
			...
			Resource resource = repositoryService.getResource(exContext(context), resourceUri);
			...
			.append("\"date\":\"").append(resource.getCreationDate()).append("\",")
		...
		try {
			...
			if (folderUri.length() > 1) {   
				while ((fromIndex = folderUri.indexOf('/', fromIndex)) != -1) {
					...
					String displayLabel = (repositoryService.getFolder(exContext(context), currentUri)).getLabel();
					fromIndex++;
					sb.append(",\"" + escape(currentUri) + "\":\"").append(escape(displayLabel)).append("\"");
				}
			}
			Resource res = repositoryService.getFolder(exContext(context), folderUri);
	...
	public Event copyResource(RequestContext context) {
		try {
			...
			while (str.hasMoreElements()) {
				String currentResource = (String)str.nextElement();
				int lastIndex = currentResource.lastIndexOf("/");
				if (lastIndex != -1) {
					currentDestUri = destUri + "/" + currentResource.substring(lastIndex + 1);
				}
				try {
					resource = repositoryService.copyResource(null, currentResource, currentDestUri);
				} catch (Exception e) {
					e.printStackTrace();
					sb.append("\"status\":\"FAILED\"");
					sb.append('}');	
			...
			sb.append("\"status\":\"SUCCESS\",");
			sb.append("\"id\":\"" + resource.getURIString() + "\"");
			sb.append('}');
			context.getRequestScope().put(AJAX_REPORT_MODEL, sb.toString());		
		} catch (UnsupportedEncodingException e) {}
		return success();
